...
type ManagedUser = {
  ...
  billingModeSetAt?: string | null; billingModeSetBy?: string | null;
  billingModeSetSource?: 'system_default' | 'master_admin';
...
const formatDateOnly = (value?: string | null) => value
  ...
  : 'Not set';
const billingModeLabel = (value?: ManagedUser['billingMode'] | null) => {
  if (value === 'meta_direct') return 'Meta direct';
  if (value === 'airavata_credits') return 'Airavata credits';
  return 'Unknown';
};
const billingModeClass = (value?: ManagedUser['billingMode'] | null) => {
  if (value === 'meta_direct') return 'bg-blue-100 text-blue-700';
  if (value === 'airavata_credits') return 'bg-emerald-100 text-emerald-700';
...
const blankForm: UserForm = {
  ...
  permissions: PERMISSIONS.map(([value]) => value), billingMode: 'unknown',
};
...
export default function MasterAdmin() {
  ...
  const openEdit = (user: ManagedUser) => {
    setSelected(user);
    setForm({ businessName: user.businessName, email: user.email, phone: normalizePhoneInput(user.phone ?? '').slice(-10), serviceStartDate: user.serviceStartDate ?? '', servicePaidThroughDate: user.servicePaidThroughDate ?? '', password: '', role: user.role, active: user.active, permissions: user.permissions, billingMode: user.billingMode ?? 'unknown' });
    setShowForm(true);
  };
